...
async def test_opposite_signal_closes_and_reverses_in_paper_mode() -> None:
    # Keep the reversal comfortably beyond the strategy's 0.5% momentum threshold.
    feed = FakeFeed([100.0, 100.7, 90.0])
    trader = LivePaperTrader(feed, TraderCouncil([MomentumStrategy()]), quantity=1.0)
    await trader.step()
    feed.candles = feed.candles[:2] + [
        Candle(
            "BTCUSDT",
            "1",
            feed.candles[1].timestamp + timedelta(minutes=1),
            90.0,
            90.0,
            90.0,
            90.0,
