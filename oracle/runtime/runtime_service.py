...
from oracle.runtime.preflight import PreflightChecker
...
class OracleRuntime:
    ...
    def evaluate(self, symbol: str, closes: list[float], equity: float,
                 entry: float, stop: float, snapshot: PortfolioSnapshot,
                 exposures: list[Exposure], beta_group: str,
                 stage: DeploymentStage = DeploymentStage.PAPER) -> RuntimeResult:
        decision = self.pipeline.evaluate(symbol, closes, equity, entry, stop)
        if not decision.approved:
            return RuntimeResult(False, decision.reason)
        risk = self.risk.approve(equity, entry, stop, snapshot, exposures, symbol, beta_group)
