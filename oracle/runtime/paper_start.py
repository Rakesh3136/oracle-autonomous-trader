...
class PaperStartup:
    def start(self) -> StartupResult:
        ...
        if not health.healthy:
            return StartupResult(False, DeploymentStage.PAPER, "integration health failed: " + "; ".join(health.failures))
    ...
    def evaluate_once(self, symbol: str, closes: list[float], equity: float, entry: float, stop: float) -> tuple[bool, str]:
        runtime = OracleRuntime()
        snapshot = PortfolioSnapshot(equity, 0.0, 0.0, equity, 0)
