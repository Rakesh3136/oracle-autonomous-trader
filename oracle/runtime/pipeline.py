...
The pipeline composes deterministic specialists, council, thesis, and risk sizing
while keeping exchange execution behind an injected coordinator. It fails closed
on missing evidence and never enables live execution by itself.
"""
...
from oracle.intelligence.thesis import Direction, TradeThesis, ThesisEngine
...
class TraderPipeline:
    ...
    def evaluate(
        ...
        thesis = self.thesis_engine.build(
            symbol,
            direction,
            "council specialist consensus",
            decision.confidence,
            entry_low=entry,
            entry_high=entry,
            invalidation=stop,
